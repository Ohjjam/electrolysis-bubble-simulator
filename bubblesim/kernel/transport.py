...
reuse. Two notes on the physics:
  ...
  * In alkaline water electrolysis the bulk electrolyte (KOH) is not the limiting
    ...
    (then capped by a clamp) as j -> j_lim, while the solvers enforce the hard
    limit with an explicit j clamp.
...
def conc_overpotential(j, j_lim, z, T):
    ...
    transport limit is enforced by the caller's explicit j-clamp, not this term.
    """
    if j <= 0.0:
        return 0.0
    x = j / j_lim
