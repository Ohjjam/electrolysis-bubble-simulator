...
Bridges the surface current distribution (current3d) to where bubbles nucleate:
...
def total_gas_rate(total_current_A, electrode, T, P, eta_F=1.0):
    ...
    return faradaic_gas_rate(total_current_A, electrode, T, P, area=1.0, eta_F=eta_F)


def nucleation_weight(surf_current, nuc_site_mult):
    """Per-voxel nucleation preference (>=0), normalised to sum 1 over the
    reacting surface. High-current, high-site-density voxels seed first."""
