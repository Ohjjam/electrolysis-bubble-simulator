...
## 3D 서버 켜기/끄기.bat — 2026-07-14 이동

3세대 서버(:8766)를 창 없이 숨김 실행/종료하던 런처 쌍. 사용자는 `run3d.bat`
(콘솔 창 = 로그 + 창 닫으면 종료)만 쓰고, 공유는 외부 서버 호스팅으로 전환
예정이라 보관함으로 이동. 다시 쓰려면 루트로 꺼내면 그대로 동작함
(경로 탐색이 상대 방식이 아니라서 루트에 있어야 함 — 정확히는 server3d_app.py
옆에 있어야 함).
